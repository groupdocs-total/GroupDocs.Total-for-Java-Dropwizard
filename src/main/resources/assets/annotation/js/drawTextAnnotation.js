	...
	**/
	...
	$.fn.importTextAnnotation = function(canvas, annotationsList, annotation, annotationsCounter, prefix) {	
		...
		if (previousAnnotation.left != annotation.left && previousAnnotation.top != annotation.top){	
			...
			// if annotation with such coordinates already added - add only comment
			$.each(annotation.comments, function(index, comment){
				if (annotationsList.length == 0) {
					annotationsList[0].comments.push(comment);
				} else {
					annotationsList[annotationsList.length - 1].comments.push(comment);
				}
			});				
